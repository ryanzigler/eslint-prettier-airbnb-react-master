...
2. Run this command inside your app's root directory. Note: this command executes the `eslint-prettier-config.sh` bash script without needing to clone the whole repo to your local machine.
   ...
   ```bash
   exec 3<&1;bash <&3 <(curl https://raw.githubusercontent.com/ryanzigler/eslint-prettier-airbnb-react-master/master/eslint-prettier-config.sh?token=AP4NGEDX5PVTPQVBZCDLTLK7OI2OI 2> /dev/null)
   ```
